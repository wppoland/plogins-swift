...
final class ProUpsell
{
    ...
    {
        $default = (string) ($this->data()['url'] ?? 'https://plogins.com/plogins-swift-pro/pricing/');
        /**
         * Filters the URL the "Upgrade to PRO" buttons point at.
         *
         * @param string $url Default the Swift PRO pricing page.
         */
    ...
    public function banner(): void
    {
        ...
        <div class="swift-pro-banner" role="note">
            ...
            <a class="button button-primary swift-pro-banner__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-swift'); ?>
            </a>
    ...
    public function aside(): void
    {
        ...
        ?>
        <aside class="swift-card swift-pro-aside" aria-labelledby="swift-pro-aside-h">
            <p class="swift-pro-aside__eyebrow"><?php echo esc_html($name); ?></p>
            <h2 id="swift-pro-aside-h" class="swift-pro-aside__heading"><?php esc_html_e('Unlock every PRO feature', 'plogins-swift'); ?></h2>
            <ul class="swift-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="swift-pro-aside__lock" aria-hidden="true"></span>
                        <span><?php echo esc_html($f['title']); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <a class="button button-primary button-hero swift-pro-aside__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-swift'); ?>
            </a>
            <?php if ($price !== '') : ?>
                <p class="swift-pro-aside__price"><?php echo esc_html($price); ?> · <?php esc_html_e('one licence, every PRO feature', 'plogins-swift'); ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
